...
const LanguageDropdown = () => {
  ...
  const currentFlag = languages.find((l) => l.code === selectedLang)?.flag || Az;
  ...
  return (
    <div className="relative w-[40px]" ref={dropdownRef}>
      ...
      >
        <img src={currentFlag} alt="Selected Language" className="w-full h-full object-cover" />
      </div>
        ...
        <div className="absolute right-0 mt-2 w-28 bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200 z-50 animate-slideDown">
          {languages.map((lang) => (
            ...
            >
              ...
              <span className="capitalize text-gray-700 font-medium">{lang.code}</span>
            </div>
          ))}
        </div>
      )}
